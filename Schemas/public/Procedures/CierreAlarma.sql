-- PROCEDURE: public.cierrealarma(bigint, character varying, character varying, boolean, boolean, character varying, character varying, boolean, boolean)

-- DROP PROCEDURE IF EXISTS public.cierrealarma(bigint, character varying, character varying, boolean, boolean, character varying, character varying, boolean, boolean);
...
CREATE OR REPLACE PROCEDURE public.cierrealarma(
	...
	IN p_tipo_cierre character varying DEFAULT 'cierre_captura',
	IN p_flag_persona_encontrada boolean DEFAULT NULL,
	IN p_flag_mascota_recuperada boolean DEFAULT NULL)
LANGUAGE 'plpgsql'
AS $BODY$
...
BEGIN
	BEGIN
		...
		-- Obtener tipo_cierre real desde la tabla tipoalarma
		SELECT ta.tipo_cierre INTO v_tipo_cierre_real
		FROM tipoalarma ta WHERE ta.tipoalarma_id = v_tipoalarma_id_actual;

		-- Validar que no se intente cerrar una alarma sin_cierre
		IF v_tipo_cierre_real = 'sin_cierre' THEN
			RAISE EXCEPTION 'Denied, this alarm type does not support manual closure';
		END IF;

		-- Validar que cierre_encuesta no use este procedimiento
		IF v_tipo_cierre_real = 'cierre_encuesta' THEN
			RAISE EXCEPTION 'Denied, cierre_encuesta alarms must use ProponerCierreEncuesta procedure';
		END IF;
		...
		-- === BRANCHING POR TIPO DE CIERRE ===
		-- Solo cierre_captura ejecuta la lógica de veracidad/credibilidad/poderes
		IF v_tipo_cierre_real = 'cierre_captura' THEN
		...
		END IF; -- fin branching cierre_captura
    ...
    BEGIN
		...
		VALUES
			(
				...
				,cast(true as boolean)
				,p_flag_hubo_captura
				,p_flag_persona_encontrada
				,p_flag_mascota_recuperada
			);
