-- PROCEDURE: public.actualizardescripcionalarma(character varying, bigint, character varying, character varying, character varying, character varying, integer, character varying, character varying)

-- DROP PROCEDURE IF EXISTS public.actualizardescripcionalarma(character varying, bigint, character varying, character varying, character varying, character varying, integer, character varying, character varying);

CREATE OR REPLACE PROCEDURE public.actualizardescripcionalarma(
...
BEGIN
	...
	BEGIN
		...
		SET
			DescripcionAlarma=p_DescripcionAlarma
			,DescripcionSospechoso=p_DescripcionSospechoso
			,DescripcionVehiculo=p_DescripcionVehiculo
			,DescripcionArmas=p_DescripcionArmas
			,fechadescripcion=now()
			,latitud_originador=v_latitud_originador
			,longitud_originador=v_longitud_originador
			,ip_usuario_originador=p_IpUsuario
			,distancia_alarma_originador=v_distancia_alarma_originador
			,FlagEditado=true
			,idioma_origen=p_idioma
		WHERE
