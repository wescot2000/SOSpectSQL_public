-- PROCEDURE: public.rechazarpermisoaprotector(character varying, character varying)

-- DROP PROCEDURE IF EXISTS public.rechazarpermisoaprotector(character varying, character varying);

CREATE OR REPLACE PROCEDURE public.rechazarpermisoaprotector(
...
BEGIN
	...
	INSERT INTO 
		...
		VALUES
			(
				v_persona_id_protector
				,'El usuario '||v_login_protegido||' ha rechazado tu solicitud para seguir sus alarmas el día: '||cast(now() as timestamp with time zone)||'. No puedes continuar con el proceso de subscripción.'
				,now()
				,cast(true as boolean)
				,v_login_protegido||' ha rechazado tu solicitud para seguir sus alarmas'
				,'es'
			);
...
END;
$BODY$;
ALTER PROCEDURE public.rechazarpermisoaprotector(character varying, character varying)
    OWNER TO w4ll4c3;
